...
3. **Rehearsal: handler-level DONE; Stripe test-mode NOT VERIFIED - blocked: no Stripe test key on this machine; needs `stripe login` or an sk_test_ in Preview env.** Handler-level rehearsal ran three times against the dev server on port 3003 (`VERCEL_ENV=development`, local PGlite so no test order reached the CRM outbox, `sk_test_` placeholder key, throwaway webhook secret, fixture dir, mail capture): (a) single tier with URL, (b) pro tier pay-first (`--defer-url`), (c) standard tier pay-first driven through the real success-page form by Playwright. Each proved: signed synthetic `checkout.session.completed` -> 200 + row queued / awaiting_url; tampered body with the same signature -> 400; URL named via `/api/order-url` and a second URL refused 409; audit actually ran against https://fusiondataco.com (58/100, 5 findings, 8 pages at the time; the robots false positive below was one of the five); PDF produced (11.6 KB); hosted Blob link opened 200 `application/pdf`; `/api/order-report` opened 200 `application/pdf`; email attempted (skipped, captured to .eml). Logs: `handler-rehearsal/single/rehearsal-handler.log`, `handler-rehearsal/pro-defer-url/rehearsal-handler.log`; the signed events: `*/event.json`; PDFs: `*/report-*.pdf`; screenshots: `handler-rehearsal/success-*.png`. This is handler-level, not a Stripe test-mode run. One-command script: `npm run rehearse:checkout -- --mode handler ...` (no key) and `--mode live` (needs `STRIPE_SECRET_KEY=sk_test_...` and test price ids in the same `STRIPE_PRICE_*` vars; refuses to run on a live key). The `livemode` check now follows the key mode (`src/lib/audit-payment-proof.ts`), so a test key fulfils test sessions with no code change.
4. **/demo: DONE.** https://80-20.dev/demo, public, no login. One real run of `runDeepGrade` against https://fusiondataco.com by `npm run demo:seed` (`scripts/seed-demo.ts`): **80/100 (green), 4 findings** (medium: admin surface reachable without auth, /dashboard /admin /administrator serve a 200 client shell; medium: soft 404; low: verify rate limiting on the login endpoint; low: placeholder copy "your company"), 8 pages, 35 check groups. The first seed of the day scored 58 with a critical "robots.txt blocks the whole site" finding; that was a false positive in the generator (see the follow-up below), fixed and re-seeded, stale row and PDF deleted. Persisted three ways so the page never re-runs it: Neon `demo_reports` row `demo_p5guq5mgmu1cz02w`, the committed snapshot `public/demo/fusiondataco.json`, and the PDF `public/demo/8020-launch-audit-fusiondataco.com.pdf` (plus a Blob copy). `/api/demo` (`server/api-src/demo.ts`) serves Postgres first, snapshot second; `POST /api/demo` with the runner secret refreshes it on demand. Example buyer shown: Rob Yeager, Fusion Data Company, rob@fusiondataco.com, Single Run $79. Proof: `live-demo-mobile.png`, `live-demo-desktop.png` (retaken after the corrected seed).
...
- `a8356e4` docs: 2026-09-14 launch report, rehearsal proof, live screenshots, Lighthouse JSON
- the robots false-positive fix + corrected demo seed (sha in `git log`)
...
Created: `demo_reports` row `demo_p5guq5mgmu1cz02w` (the corrected /demo report; intended, kept). The first-seed row `demo_s5mcs13dmu1c376u` (false positive) was deleted, as was its Blob PDF copy. No `paid_audits` rows were created in Neon (rehearsals ran on local PGlite). Blob: three rehearsal PDFs under `launchaudit/orders/pa_*` were deleted after the run; the demo PDF copy under `launchaudit/demo/` is kept.
...
- Generator bug found and fixed through the demo run: the robots.txt launch check used a regex that scanned from `User-agent: *` to ANY later `Disallow: /`, so fusiondataco.com's file (Allow: / for everyone, path-specific Disallows, and a `Disallow: /` under a named Bytespider group 340 lines later) was reported as a site-wide block. Replaced with a group-aware RFC 9309 parser (`src/lib/robots.ts`): a crawler obeys only the group that names it or `*`, `Allow: /` beats path Disallows, blank Disallow allows all, comments/BOM/CRLF handled, named AI-crawler groups are informational. The live file is a test fixture (`src/lib/fixtures/robots-fusiondataco.txt`, `src/lib/robots.test.ts`). This also affects the free scan's paid sibling for every future buyer.
