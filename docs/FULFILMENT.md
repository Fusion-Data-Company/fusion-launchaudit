...
3. The buyer lands on `/order/success?session_id=cs_...`. That page polls `/api/order-status`, and the first poll that finds a `queued` row runs the site-wide grade right there. If nobody polls (tab closed), `POST /api/grade-order` with `Authorization: Bearer $RUNNER_SYNC_SECRET` sweeps queued rows.
