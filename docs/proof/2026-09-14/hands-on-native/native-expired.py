...
run('native-owner-a@example.invalid', [['js',"window.results=null;Promise.all([fetch('/settings',{method:'POST',headers:{'Next-Action':'40fa8bd2d02b16fbe96155554d8ed8e157ca8e613b','Content-Type':'text/plain'},body:JSON.stringify(['FORBIDDEN EXPIRED CHANGE'])}),fetch('/api/account/export'),fetch('/refunds')].map(async r=>{r=await r;return {status:r.status,body:(await r.text()).slice(0,500)}})).then(r=>window.results=r);'started'"],wait,['js','JSON.stringify(window.results)']], 'native-expired-export.log')
