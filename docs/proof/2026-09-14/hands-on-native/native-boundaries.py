...
run('native-owner-b@example.invalid', [['js',"window.results=null;Promise.all([101,201].map(async id=>{let r=await fetch('/api/receipts/'+id);return {id,status:r.status,body:(await r.text()).slice(0,100)}})).then(r=>window.results=r);'started'"],wait,['js','JSON.stringify(window.results)']], 'native-account-b.log')
