 ...
 * No browser, no repo, no code upload — it only touches the public URL the
 * visitor pastes (like any web scanner), so it is privacy-safe and runs in a
 * serverless function. It is an honest SUBSET of the full local audit; the deep
 * checks (IDOR, admin/RBAC, repo) still run in the user's own agent.
 */
...
type Finding = { title: string; severity: Sev; detail: string; category: string };
...
async function assertPublic(host: string) {
  ...
  if (net.isIP(host)) { if (privateIp(host)) throw new Error("private ip"); return; }
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!raw) { response.status(400).json({ error: "Provide a url." }); return; }
  if (!/^https?:\/\//i.test(raw)) raw = "https://" + raw;
  let u: URL;
  try { u = new URL(raw); } catch { response.status(400).json({ error: `Not a valid URL: ${raw}` }); return; }
  if (u.protocol !== "http:" && u.protocol !== "https:") { response.status(400).json({ error: "Only http/https URLs." }); return; }
  try { await assertPublic(u.hostname); } catch { response.status(400).json({ error: "That host isn't a public address we can scan." }); return; }
  ...
  try {
    ...
    response.status(502).json({ error: `Couldn't reach ${u.origin}. Make sure it's live and public.` }); return;
  }
